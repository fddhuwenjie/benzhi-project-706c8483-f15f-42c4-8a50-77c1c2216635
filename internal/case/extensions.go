...
type inspectionReportCacheEntry struct {
	revision int64
...
func cloneInspectionReport(report InspectionReport) InspectionReport {
	result := report
	result.IndependentReadings = append([]store.EnvironmentReading(nil), report.IndependentReadings...)
	result.Failures = append([]string(nil), report.Failures...)
	result.MissingReadings = append([]string(nil), report.MissingReadings...)
	result.HypothesesToReview = append([]string(nil), report.HypothesesToReview...)
	result.IsolationGaps = append([]string(nil), report.IsolationGaps...)
	return result
}
...
func (s *Service) InspectionReport(incidentID string) (InspectionReport, error) {
	...
	s.inspectionReportMu.Lock()
	defer s.inspectionReportMu.Unlock()
	if cached, ok := s.inspectionReportCache[incidentID]; ok && cached.revision == incident.Revision {
		return cloneInspectionReport(cached.report), nil
	}
	...
	s.inspectionReportCache[incidentID] = inspectionReportCacheEntry{revision: incident.Revision, report: cloneInspectionReport(r)}
	return cloneInspectionReport(r), nil
}
