...
type planDiffCacheEntry struct {
	IncidentID string
	Revision   int64
	View       PlanDiffView
}
...
func (s *Service) PlanDiff(incidentID string, fromVersion, toVersion int) (PlanDiffView, error) {
	...
	s.planDiffMu.Lock()
	if cached := s.planDiffCache; cached != nil && cached.IncidentID == incidentID && cached.Revision == incident.Revision {
		view := cached.View
		s.planDiffMu.Unlock()
		return view, nil
	}
	...
	s.planDiffMu.Lock()
	s.planDiffCache = &planDiffCacheEntry{IncidentID: incidentID, Revision: incident.Revision, View: view}
	s.planDiffMu.Unlock()
	return view, nil
}
