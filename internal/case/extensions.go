...
func cloneVerificationHistoryView(view VerificationHistoryView) VerificationHistoryView {
	result := view
	result.Suggestions = append([]string(nil), view.Suggestions...)
	result.Rounds = append([]store.RecoveryVerification(nil), view.Rounds...)
	for i := range result.Rounds {
		result.Rounds[i].Failures = append([]string(nil), view.Rounds[i].Failures...)
		result.Rounds[i].FailureDetails = append([]store.VerificationFailure(nil), view.Rounds[i].FailureDetails...)
	}
	return result
}
...
func (s *Service) VerificationHistory(incidentID string) (VerificationHistoryView, error) {
	...
	s.verificationHistoryMu.RLock()
	cached, ok := s.verificationHistoryCache[incident.Revision]
	s.verificationHistoryMu.RUnlock()
	if ok {
		return cloneVerificationHistoryView(cached), nil
	}
	...
	s.verificationHistoryMu.Lock()
	s.verificationHistoryCache[incident.Revision] = cloneVerificationHistoryView(view)
	s.verificationHistoryMu.Unlock()
	return cloneVerificationHistoryView(view), nil
}
