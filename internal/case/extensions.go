...
type commitmentCompletionStoreResult struct {
	incident *store.EnvironmentIncident
	replayed bool
	failure  string
}

func captureCommitmentCompletion(result *store.EnvironmentIncident, replayed bool, err error) commitmentCompletionStoreResult {
	staged := commitmentCompletionStoreResult{incident: result, replayed: replayed}
	if err != nil {
		staged.failure = err.Error()
	}
	return staged
}

func finalizeCommitmentCompletion(staged commitmentCompletionStoreResult) (*store.EnvironmentIncident, bool, error) {
	if staged.failure != "" {
		return nil, false, errors.New(staged.failure)
	}
	return staged.incident, staged.replayed, nil
}
...
func (s *Service) CompleteDeadlineCommitment(incidentID, commitmentID string, input CompleteCommitmentInput) (*store.EnvironmentIncident, bool, error) {
	...
	now := s.now()
	result, replayed, err := s.update(incidentID, input.Meta, "complete_deadline_commitment", "deadline.commitment_completed", input, func(incident *store.EnvironmentIncident) error {
	...
	return finalizeCommitmentCompletion(captureCommitmentCompletion(result, replayed, err))
}
