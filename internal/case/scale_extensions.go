...
func (s *Service) FollowUps(id, conclusion string, now time.Time) (FollowUpView, error) {
	...
	cacheKey := fmt.Sprintf("%s\x00%d\x00%s", id, in.Revision, conclusion)
	s.followUpMu.Lock()
	if s.followUpCache == nil {
		s.followUpCache = make(map[string]FollowUpView)
	}
	if cached, ok := s.followUpCache[cacheKey]; ok {
		s.followUpMu.Unlock()
		return cached, nil
	}
	s.followUpMu.Unlock()
