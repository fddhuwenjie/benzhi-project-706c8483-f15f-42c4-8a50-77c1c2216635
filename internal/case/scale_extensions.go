...
func (s *Service) ContextSnapshot(id string) (map[string]any, error) {
	if cached, ok := s.contextSnapshotCache.Load(id); ok {
		return contextSnapshotResponse(cached.(*contextSnapshotCacheEntry)), nil
	}
	in, err := s.Get(id)
	if err != nil {
		return nil, err
	}
	entry := &contextSnapshotCacheEntry{
		incidentID:     id,
		revision:       in.Revision,
		currentVersion: in.CurrentContextVersion,
		snapshots:      append([]store.ContextSnapshot(nil), in.ContextSnapshots...),
	}
	s.contextSnapshotCache.Store(id, entry)
	return contextSnapshotResponse(entry), nil
}

func contextSnapshotResponse(entry *contextSnapshotCacheEntry) map[string]any {
	return map[string]any{
		"incident_id":     entry.incidentID,
		"current_version": entry.currentVersion,
		"snapshots":       append([]store.ContextSnapshot(nil), entry.snapshots...),
	}
}
