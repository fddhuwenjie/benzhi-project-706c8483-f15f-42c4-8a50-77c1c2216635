...
func (s *Service) MaterialBatchTracking(batch string) ([]MaterialBatchSummary, error) {
	...
	for _, incident := range incidents {
		version.WriteString(incident.IncidentID)
		version.WriteByte(':')
		version.WriteString(strconv.FormatInt(incident.Revision, 10))
		version.WriteByte(';')
	}

	s.materialCacheMu.Lock()
	defer s.materialCacheMu.Unlock()
	if s.materialCache.fingerprint != version.String() {
		all := []MaterialBatchSummary{}
		for _, incident := range incidents {
			for _, execution := range incident.Executions {
				for _, material := range execution.Materials {
					all = append(all, MaterialBatchSummary{BatchNumber: material.BatchNumber, Name: material.Name, LatestExpiresAt: material.ExpiresAt, Operators: []string{execution.OperatorID}, QuantityValues: []any{material.Quantity}, ExecutionIDs: []string{execution.ExecutionID}, PlanVersions: []int{execution.PlanVersion}, UseCount: 1})
				}
			}
		}
	...
	for _, item := range items {
		if strings.EqualFold(strings.TrimSpace(item.BatchNumber), strings.TrimSpace(batch)) {
