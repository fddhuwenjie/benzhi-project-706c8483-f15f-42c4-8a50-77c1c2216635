...
func (s *Service) ReassessmentTasks(id string, status string, owner string, from, to time.Time) ([]store.ReassessmentTask, error) {
	...
	if s.reassessmentCache != nil {
		if cached, ok := s.reassessmentCache[id]; ok {
			result := append([]store.ReassessmentTask(nil), cached...)
	...
	s.reassessmentCacheMu.Lock()
	if s.reassessmentCache == nil {
		s.reassessmentCache = make(map[string][]store.ReassessmentTask)
	}
	s.reassessmentCache[id] = append([]store.ReassessmentTask(nil), out...)
	s.reassessmentCacheMu.Unlock()
	return out, nil
}
