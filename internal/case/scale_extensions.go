...
type reopenPreviewCacheEntry struct {
	incidentID string
	revision   int64
	preview    ReopenPreview
}

func cloneReopenPreview(value ReopenPreview) ReopenPreview {
	cloned := value
	cloned.Checks = append([]rules.ReadinessItem(nil), value.Checks...)
	return cloned
}
...
func (s *Service) ReopenPreview(id, role string, revision int64) (ReopenPreview, error) {
	...
	s.reopenPreviewMu.RLock()
	cached := s.reopenPreviewCache
	if cached != nil &&
		cached.incidentID == id &&
		cached.revision == in.Revision {
		preview := cloneReopenPreview(cached.preview)
		s.reopenPreviewMu.RUnlock()
		return preview, nil
	}
	s.reopenPreviewMu.RUnlock()
	checkedAt := s.now()
	...
	s.reopenPreviewMu.Lock()
	s.reopenPreviewCache = &reopenPreviewCacheEntry{
		incidentID: id,
		revision:   in.Revision,
		preview:    cloneReopenPreview(p),
	}
	s.reopenPreviewMu.Unlock()
	return p, nil
}
