...
type renewalCommitResult struct {
	incident *store.EnvironmentIncident
	replayed bool
	failure  string
}

func stageRenewalCommit(incident *store.EnvironmentIncident, replayed bool, err error) renewalCommitResult {
	result := renewalCommitResult{incident: incident, replayed: replayed}
	if err != nil {
...
func finalizeRenewalCommit(result renewalCommitResult) (*store.EnvironmentIncident, bool, error) {
	if result.failure != "" {
...
func (s *Service) RenewCommitment(id, cid string, in RenewInput) (*store.EnvironmentIncident, bool, error) {
	...
	now := s.now()
	incident, replayed, err := s.update(id, in.Meta, "renew_deadline_commitment", "deadline.commitment_renewed", in, func(x *store.EnvironmentIncident) error {
		for i := range x.DeadlineCommitments {
			c := &x.DeadlineCommitments[i]
			if c.CommitmentID == cid {
				if c.Status != "expired" {
	...
	return finalizeRenewalCommit(stageRenewalCommit(incident, replayed, err))
}
