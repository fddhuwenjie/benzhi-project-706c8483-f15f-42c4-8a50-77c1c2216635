...
func (s *Service) PlanPreflight(id string, input SubmitPlanInput) (PlanPreflightView, error) {
	...
	s.planPreflightMu.Lock()
	if cached := s.planPreflightHit; cached != nil && cached.incidentID == id && cached.revision == in.Revision {
		out := cached.view
		s.planPreflightMu.Unlock()
		return out, nil
	}
	s.planPreflightMu.Unlock()
	...
	s.planPreflightMu.Lock()
	s.planPreflightHit = &planPreflightCacheEntry{incidentID: id, revision: in.Revision, view: out}
	s.planPreflightMu.Unlock()
	return out, nil
}
