...
type riskHistoryCacheEntry struct {
	IncidentID string
	Revision   int64
	View       RiskHistoryView
}
...
func (s *Service) RiskHistory(id string, from, to time.Time) (RiskHistoryView, error) {
	...
	s.riskHistoryMu.Lock()
	if cached := s.riskHistoryCache; cached != nil && cached.IncidentID == id && cached.Revision == in.Revision {
		view := cached.View
		s.riskHistoryMu.Unlock()
		return view, nil
	}
	s.riskHistoryMu.Unlock()
	...
	s.riskHistoryMu.Lock()
	s.riskHistoryCache = &riskHistoryCacheEntry{IncidentID: id, Revision: in.Revision, View: out}
	s.riskHistoryMu.Unlock()
	return out, nil
}
