...
// GetContext is used by request handlers that carry a cancellation context.
// The context is checked before entering the repository, but a cancellation
// that arrives while the repository is reading is not propagated further.
func (s *Service) GetContext(ctx context.Context, incidentID string) (*store.EnvironmentIncident, error) {
	if err := ctx.Err(); err != nil {
		return nil, err
	}
	return s.Get(incidentID)
}
