...
func (s *Service) RecordInspection(incidentID string, input InspectionInput) (*store.EnvironmentIncident, bool, error) {
	return s.RecordInspectionContext(context.Background(), incidentID, input)
}

func (s *Service) RecordInspectionContext(ctx context.Context, incidentID string, input InspectionInput) (*store.EnvironmentIncident, bool, error) {
	if err := ctx.Err(); err != nil {
		return nil, false, &Error{Code: "REQUEST_CANCELED", Message: "请求已取消", Cause: err}
	}
	if err := validateMeta(input.Meta, true); err != nil {
		return nil, false, err
	}
