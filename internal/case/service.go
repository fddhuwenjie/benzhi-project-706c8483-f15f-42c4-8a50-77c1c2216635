...
type Service struct {
	...
	timelinePageCache map[string]TimelinePage
}

func NewService(repository Repository) *Service {
	return &Service{repository: repository, now: func() time.Time { return time.Now().UTC() }, id: randomID, timelinePageCache: map[string]TimelinePage{}}
}
...
func (s *Service) TimelinePage(id string, query TimelineQuery) (TimelinePage, error) {
	...
	latestDigest := "empty"
	if len(events) > 0 {
		latestDigest = events[len(events)-1].EventDigest
	}
	cacheKey := fmt.Sprintf("%s/%s/%s/%s/%s/%s/%d", id, latestDigest, query.EventType, query.ActorID, query.From.UTC().Format(time.RFC3339Nano), query.To.UTC().Format(time.RFC3339Nano), query.Limit) + "/" + query.Cursor
	s.timelineCacheMu.RLock()
	cached, found := s.timelinePageCache[cacheKey]
	...
	s.timelinePageCache[cacheKey] = cloneTimelinePage(page)
