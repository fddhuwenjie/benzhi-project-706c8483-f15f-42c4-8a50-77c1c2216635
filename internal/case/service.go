...
type Service struct {
	...
	queryCache   map[string]store.IncidentPage
}

func NewService(repository Repository) *Service {
	return &Service{repository: repository, now: func() time.Time { return time.Now().UTC() }, id: randomID, queryCache: map[string]store.IncidentPage{}}
}
...
func (s *Service) Query(input ListQuery) (store.IncidentPage, error) {
	...
	cacheKey := fmt.Sprintf("%d|%s", input.Limit, input.Cursor)
	s.queryCacheMu.Lock()
	if cached, ok := s.queryCache[cacheKey]; ok {
		s.queryCacheMu.Unlock()
		return cloneIncidentPage(cached)
	}
	s.queryCacheMu.Unlock()
	...
	cached, err := cloneIncidentPage(page)
	if err != nil {
		return store.IncidentPage{}, err
	}
	s.queryCacheMu.Lock()
	s.queryCache[cacheKey] = cached
	s.queryCacheMu.Unlock()
	return cloneIncidentPage(page)
}

func cloneIncidentPage(page store.IncidentPage) (store.IncidentPage, error) {
	data, err := json.Marshal(page)
	if err != nil {
		return store.IncidentPage{}, err
	}
	var result store.IncidentPage
