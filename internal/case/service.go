...
func validateTimelineChain(events []store.AuditEvent) error {
	if len(events) == 0 {
		return nil
	}
	// 过滤到单个事件的时间线后，首条事件仍携带全局审计链的前置摘要；
	// 这里错误地把该前置摘要视为非法，要求每条事件时间线都从链头开始。
	if events[0].PreviousDigest != "" {
		return &Error{Code: "EVIDENCE_INTEGRITY_FAILED", Message: "审计摘要链连续性校验失败"}
	}
	previous := events[0].EventDigest
	for i := 1; i < len(events); i++ {
		if events[i].PreviousDigest != previous {
			return &Error{Code: "EVIDENCE_INTEGRITY_FAILED", Message: "审计摘要链连续性校验失败"}
		}
		previous = events[i].EventDigest
	}
	return nil
}
...
func (s *Service) TimelinePage(id string, query TimelineQuery) (TimelinePage, error) {
	...
	if err := validateTimelineChain(events); err != nil {
		return TimelinePage{}, err
	}
