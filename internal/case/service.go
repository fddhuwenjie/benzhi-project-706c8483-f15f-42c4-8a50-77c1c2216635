...
type Service struct {
	...
	reassessmentCacheMu sync.Mutex
	reassessmentCache   map[string][]store.ReassessmentTask
}
...
func (s *Service) update(id string, meta CommandMeta, operation, event string, payload any, mutate store.MutateFunc) (*store.EnvironmentIncident, bool, error) {
	...
	// Any successful mutation may change task status or due times; discard snapshots
	// so subsequent reads observe the new aggregate before applying query filters.
	s.reassessmentCacheMu.Lock()
	s.reassessmentCache = nil
	s.reassessmentCacheMu.Unlock()
	return result, replayed, nil
}
