...
type planReviewStoreResult struct {
	incident *store.EnvironmentIncident
	replayed bool
	failure  string
}

type planReviewCommitResult struct {
	incident *store.EnvironmentIncident
	replayed bool
	failure  string
}

func capturePlanReviewStoreResult(run func() (*store.EnvironmentIncident, bool, error)) planReviewStoreResult {
	incident, replayed, err := run()
	if err != nil {
		return planReviewStoreResult{failure: err.Error()}
	}
	return planReviewStoreResult{incident: incident, replayed: replayed}
}

func stagePlanReviewCommit(result planReviewStoreResult) planReviewCommitResult {
	if result.failure != "" {
		return planReviewCommitResult{failure: result.failure}
	}
	return planReviewCommitResult{incident: result.incident, replayed: result.replayed}
}

func finalizePlanReviewCommit(result planReviewCommitResult) (*store.EnvironmentIncident, bool, error) {
	if result.failure != "" {
		return nil, false, errors.New(result.failure)
	}
	if result.incident == nil {
		return nil, false, errors.New("方案审核提交结果为空")
	}
	return result.incident, result.replayed, nil
}
...
func (s *Service) ReviewPlan(incidentID string, input ReviewPlanInput) (*store.EnvironmentIncident, bool, error) {
	...
	return finalizePlanReviewCommit(stagePlanReviewCommit(capturePlanReviewStoreResult(func() (*store.EnvironmentIncident, bool, error) {
