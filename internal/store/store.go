...
func (s *Store) Evidence(incidentID string) (EvidenceSummary, error) {
	...
	summary.EvidenceGaps = evidenceGaps(incident)
	summary.Manifest = cloneEvidenceManifest(incident.EvidenceManifest)
	return summary, nil
}

// cloneEvidenceManifest 隔离证据清单响应与持久化快照的所有权。
func cloneEvidenceManifest(manifest *EvidenceManifest) *EvidenceManifest {
	if manifest == nil {
		return nil
	}
