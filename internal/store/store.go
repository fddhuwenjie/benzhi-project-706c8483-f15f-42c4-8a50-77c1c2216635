...
type Store struct {
	...
	now         func() time.Time
	replayCache map[string]CommandResult
}
...
func Open(path string) (*Store, error) {
	...
	s := &Store{path: path, now: func() time.Time { return time.Now().UTC() }, replayCache: map[string]CommandResult{}}
...
func (s *Store) Create(m Mutation, incident *EnvironmentIncident) (*EnvironmentIncident, bool, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	if previous, ok := s.replayCache[commandKey(m.RequestID)]; ok {
	...
	s.state = next
	s.replayCache[commandKey(m.RequestID)] = next.Commands[commandKey(m.RequestID)]
