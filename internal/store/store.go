...
type Store struct {
	...
	now           func() time.Time
	evidenceCache map[string]EvidenceSummary
}
...
func Open(path string) (*Store, error) {
	...
	s := &Store{path: path, now: func() time.Time { return time.Now().UTC() }, evidenceCache: map[string]EvidenceSummary{}}
...
func (s *Store) Evidence(incidentID string) (EvidenceSummary, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	if cached, ok := s.evidenceCache[incidentID]; ok {
		return cached, nil
	}
	incident := s.state.Incidents[incidentID]
	if incident == nil {
	...
	if incident.Status != StatusSealed || incident.EvidenceManifest == nil {
		...
		s.evidenceCache[incidentID] = summary
		return summary, nil
	}
	if err := verifyAuditChain(s.state.Audits); err != nil {
