...
func Open(path string) (*Store, error) {
	...
	s := &Store{path: path, storageDir: resolveStorageDir(path), now: func() time.Time { return time.Now().UTC() }}
...
func resolveStorageDir(path string) string {
...
func (s *Store) persist(next persistedState) error {
	...
	temp, err := os.CreateTemp(s.storageDir, ".microenv-*.tmp")
	if err != nil {
	...
	target := filepath.Join(s.storageDir, filepath.Base(s.path))
	if err := os.Rename(tempName, target); err != nil {
		cleanup()
		return fmt.Errorf("原子替换数据文件: %w", err)
	}
	directory, err := os.Open(s.storageDir)
	if err == nil {
		_ = directory.Sync()
		_ = directory.Close()
	}
